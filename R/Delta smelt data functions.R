#################################################################
### Data summary code for IBMR ##################################
### revised by William Smith (USFWS; BDFWO) 18 Mar 2021
...
WQ_dat=read.csv(file='delta_water_quality_data_with_strata_v2.csv',header=T,stringsAsFactors = F)
...
DS.dist=read.table('DS_distribution_JanDec_1995_2015.txt',header=T)
...
for (j in c(1:12)) {
	ifelse(Month[i]==1,temp.distr[(1:12),j,i] <- as.matrix(DS.dist[which(Year==1979+i+1 & Month==j),(3:14)]), # January is the 12th month of the life cycle, so it gets the next year's value
		temp.distr[(1:12),j,i] <- as.matrix(DS.dist[which(Year==1979+i & Month==j),(3:14)]))
	for (k in 1:12) {
		temp.distr[k,j,i] <- vol[k]*temp.distr[k,j,i] # volumetric expansion to abundance estimate
		}}}
for (i in first:length(yearz)) {
for (j in c(1:12)) {
	for (k in 1:12) {
		distr[k,j,i] <- max(0.001,(temp.distr[k,j,i]/sum(temp.distr[(1:n.strata),j,i]))) # estimate occupancy probability (non-zero) 
		}}}

smlt.dist<-function(t,yr){
if(t==1) { prbs<-distr[,1,yr]; prbs[3]<-prbs[3]/(1-PEL[which(PEL.Year==1979+yr),'Jan']) } # January observations
if(t==2) { prbs<-distr[,2,yr]; prbs[3]<-prbs[3]/(1-PEL[which(PEL.Year==1979+yr),'Feb']) } # February observations
if(t==3) { prbs<-as.vector(distr[,3,yr]); prbs[3]<-prbs[3]/(1-PEL[which(PEL.Year==1979+yr),'Mar']) } # March observations
if(t==4) { prbs<-distr[,4,yr]; prbs[3]<-prbs[3]/(1-PEL[which(PEL.Year==1979+yr),'Apr']) }  # April observations
if(t==5) { prbs<-distr[,5,yr]; prbs[3]<-prbs[3]/(1-PEL[which(PEL.Year==1979+yr),'May']) }  # May observations
if(t==6) { prbs<-distr[,6,yr]; prbs[3]<-prbs[3]/(1-PEL[which(PEL.Year==1979+yr),'Jun']) }  # June observations
...
if(t==12) { prbs<-distr[,12,yr]; prbs[3]<-prbs[3]/(1-PEL[which(PEL.Year==1979+yr),'Dec']) }  # December observations
...
zoopdat1=read.table('Zoop.limno_____.txt',header=T)
zoopdat2=read.table('Zoop.othcaljuv_.txt',header=T)
zoopdat3=read.table('Zoop.pdiapjuv__.txt',header=T)
zoopdat4=read.table('Zoop.othcalad__.txt',header=T)
zoopdat5=read.table('Zoop.acartela__.txt',header=T)
zoopdat6=read.table('Zoop.othclad___.txt',header=T)
zoopdat7=read.table('Zoop.allcopnaup.txt',header=T)
zoopdat8=read.table('Zoop.daphnia___.txt',header=T)
zoopdat9=read.table('Zoop.othcyc____.txt',header=T)
zoopdat10=read.table('Zoop.other_____.txt',header=T)
zoopdat11=read.table('Zoop.eurytem___.txt',header=T)
zoopdat12=read.table('Zoop.pdiapfor__.txt',header=T)
...
zoopP01=read.table('Zoop.P0.limno_____.txt',header=T)
zoopP02=read.table('Zoop.P0.othcaljuv_.txt',header=T)
zoopP03=read.table('Zoop.P0.pdiapjuv__.txt',header=T)
zoopP04=read.table('Zoop.P0.othcalad__.txt',header=T)
zoopP05=read.table('Zoop.P0.acartela__.txt',header=T)
zoopP06=read.table('Zoop.P0.othclad___.txt',header=T)
zoopP07=read.table('Zoop.P0.allcopnaup.txt',header=T)
zoopP08=read.table('Zoop.P0.daphnia___.txt',header=T)
zoopP09=read.table('Zoop.P0.othcyc____.txt',header=T)
zoopP010=read.table('Zoop.P0.other_____.txt',header=T)
zoopP011=read.table('Zoop.P0.eurytem___.txt',header=T)
zoopP012=read.table('Zoop.P0.pdiapfor__.txt',header=T)
...
for (mn in 1:12) { # get number of days in each month, varies in leap years
 dayz.temp[mn] <- days.in.month(yearz[yr],mn)
 }
for (h in 1:12) {
for (i in 1:n.prey) {
...
tempDSM=read.table('temp.txt',header=T)

# Model to predict surface temps as a function of DSM2 temps for 1995-2010
...
make.temp<-function(t,yr) {
 ...
 if (t == 1) { temp.hldr[h,1] <- Temp_table.DSM2.mean[(yr-9),t,h] # January observations # January is the 12th month of the life cycle, so it gets the next year's value
  temp.hldr[h,2] <- Temp_table.DSM2.sd[(yr-9),t,h] }
  else { temp.hldr[h,1] <- Temp_table.DSM2.mean[(yr-10),t,h] 
  ...
  if (t == 1) { temp.hldr[h,1] <- Surv2DSM[(yr-30),t,h] # January observations # January is the 12th month of the life cycle, so it gets the next year's value
   temp.hldr[h,2] <- mean(Temp_table.DSM2.sd[,t,h],na.rm=T) } # use month-strata mean sd for 2011-2014
  else { temp.hldr[h,1] <- Surv2DSM[(yr-31),t,h] 
...
#################################################################
# 5. Calculate egg to larvae survival based on Bennett 2005
